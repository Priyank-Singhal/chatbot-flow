...
const CustomNode = ({ data, id }) => {
    ...
    return (
        <div className="shadow-lg rounded-md bg-white border-2 border-stone-400 w-44">
            ...
            <div className="ml-2" onClick={handleNodeClick}>
                <div className="text-gray-500">{data.msg}</div>
            </div>
            <Handle type="target" position={Position.Left} className="w-16" />
            <Handle type="source" position={Position.Right} className="w-16" />
        </div>
    );
}
